...
function Svhn:__init(config)
   ...
   load_all, input_preprocess, target_preprocess
      ...
      {arg='download_url', type='string',
       default='http://data.neuflow.org/data/svhn.t7.tgz',
...
function Svhn:_loadTrainValid(file_name, valid_ratio, valid_per_class)
   ...
   local data = torch.load(doc_path,'ascii'):float() -- was ByteTensor
   local inputs = train.X:transpose(3,4)
   local targets = train.y:view(-1)
   ...
   if valid_ratio then
      ...
      for class, indices in ipairs(class_indices) do
         local cidx = torch.LongTensor(indices)
         -- valid
         local vidx = cidx:narrow(1, 1, valid_per_class)
         local vinput = valid_inputs:narrow(1, vstart_, valid_per_class)
         local vtarget = valid_inputs:narrow(1, vstart_, valid_per_class)
         ...
         local tindices = indices:narrow(1, tstart_, valid_per_class)
      ...
      assert(vstart_ == valid_targets:size(1) + 1)
      assert(tstart_ == nTrain - 1)
...
function Svhn:loadTrainValid()
   ...
   if self._use_extra then
      self:_loadTrainValid(
         self._extra_file, 
         self._valid_ratio = self._valid_ratio[2], 
         self._valid_per_class = self._valid_per_class[2]
      )
   end
   
   assert(math.min(train_inputs:min(), train_targets:min()) > -1)
   assert(math.min(valid_inputs:min(), valid_targets:min()) > -1)
