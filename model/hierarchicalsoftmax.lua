...
------------------------------------------------------------------------
local HierarchicalSoftmax, parent = torch.class("dp.HierarchicalSoftmax", "dp.Model")
HierarchicalSoftmax.isHierarchicalSoftmax = true

function HierarchicalSoftmax:__init(config)
   ...
   local args, input_size, output_size, transfer, dropout, typename, 
      ...
      {arg='transfer', type='nn.Module', req=true,
       help='a transfer function like nn.SoftMax, nn.LogSoftMax '},
   ...
   self._sol = nn.SparseOutLinear(input_size, output_size, setup, dynamic)
...
function HierarchicalSoftmax:_forward(carry)
...
function HierarchicalSoftmax:_backward(carry)
...
function HierarchicalSoftmax:_accept(visitor)
...
function HierarchicalSoftmax:zeroGradParameters()
   self._affine:zeroGradParameters()
end

function HierarchicalSoftmax:type(type)
...
function HierarchicalSoftmax:reset()
   self._affine:reset()
...
function HierarchicalSoftmax:parameters()
   local params = {}
...
function HierarchicalSoftmax:maxNorm(max_out_norm, max_in_norm)
...
function HierarchicalSoftmax:share(neural, ...)
   assert(neural.isHierarchicalSoftmax)
   local arg = {...}
...
function HierarchicalSoftmax:sharedClone()
   local clone = self:clone()
   return self:share(clone, 'weight', 'bias')
end

function HierarchicalSoftmax:report()
...
function HierarchicalSoftmax._sparseReset(W, stdev)
   assert(W:dim() == 2, 
